...
export default function AulaPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      >
        <div className="text-center">
          <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center mx-auto mb-3">
            <Play size={28} strokeWidth={1.75} className="text-white/80 ml-1" fill="currentColor" />
          </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-muted">
            <span className="flex items-center gap-1"><User size={12} strokeWidth={1.75} /> {aula.professor}</span>
            <span className="flex items-center gap-1"><Clock size={12} strokeWidth={1.75} /> {aula.duracao}</span>
            <span>{modulo.nome}</span>
            {concluida && (
              <span className="flex items-center gap-1 text-primary font-medium">
                <Check size={12} strokeWidth={1.75} /> Concluída
              </span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              salva ? "border-primary bg-primary/5 text-primary" : "border-line text-muted hover:border-primary"
            )}
          >
            <Bookmark size={14} strokeWidth={1.75} fill={salva ? "currentColor" : "none"} />
            {salva ? "Salva" : "Salvar"}
          </button>
          <button
            ...
            className={cn(
              ...
              concluida ? "bg-line text-muted" : "bg-primary text-surface hover:bg-primary-light active:scale-95"
            )}
          >
      ...
      {aula.galeria && aula.galeria.length > 0 && (
        <div className="card-fassi p-5">
          <h2 className="font-serif text-lg font-semibold text-ink mb-1 flex items-center gap-2">
            <ImageIcon size={18} strokeWidth={1.75} className="text-primary" /> Veja de perto
          </h2>
              ...
              >
                {img.url ? (
                  ...
                  <div className="flex flex-col items-center text-muted">
                    <ImageIcon size={24} strokeWidth={1.75} className="mb-1 opacity-40" />
          ...
          <div className="max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="rounded-xl overflow-hidden bg-bg aspect-[4/3] flex items-center justify-center mb-3">
              {aula.galeria[galeriaAberta].url ? (
                ...
                <ImageIcon size={48} strokeWidth={1.75} className="text-muted opacity-40" />
              )}
            </div>
      ...
      {aula.materiais && aula.materiais.length > 0 && (
        <div className="card-fassi p-5">
          <h2 className="font-serif text-base font-semibold text-ink mb-3 flex items-center gap-2">
            <Download size={16} strokeWidth={1.75} className="text-primary" /> Materiais para download
          </h2>
          <div className="space-y-2">
      ...
      <div className="card-fassi p-5">
        <h2 className="font-serif text-base font-semibold text-ink mb-3 flex items-center gap-2">
          <StickyNote size={16} strokeWidth={1.75} className="text-primary" /> Anotações pessoais
        </h2>
        <textarea
          value={anotacao}
      ...
      {aula.faq && aula.faq.length > 0 && (
        <div className="card-fassi p-5">
          <h2 className="font-serif text-base font-semibold text-ink mb-3 flex items-center gap-2">
            <HelpCircle size={16} strokeWidth={1.75} className="text-primary" /> Perguntas frequentes
          </h2>
          <div className="space-y-2">
            {aula.faq.map((f, i) => (
      ...
      {relacionados.length > 0 && (
        <div>
          <h2 className="font-serif text-lg font-semibold text-ink mb-3 flex items-center gap-2">
            <BookOpen size={18} strokeWidth={1.75} className="text-primary" /> Conteúdos relacionados
          </h2>
      ...
      <div className="flex items-center justify-between gap-4 pt-4 border-t border-line">
        {anterior ? (
          ...
          >
            <ArrowLeft size={16} strokeWidth={1.75} /> Aula anterior
          </Link>
        ) : (
          <span />
        )}
        {proxima ? (
          <button
            ...
            className="inline-flex items-center gap-2 bg-primary hover:bg-primary-light text-surface px-5 py-2.5 rounded-xl font-semibold text-sm transition-all active:scale-95"
          >
            Próxima aula <ArrowRight size={16} strokeWidth={1.75} />
          </button>
        ) : (
          <Link
            href={`/app/universidade/curso/${curso.id}`}
            className="inline-flex items-center gap-2 bg-primary hover:bg-primary-light text-surface px-5 py-2.5 rounded-xl font-semibold text-sm transition-all"
          >
            Concluir curso <Check size={16} strokeWidth={1.75} />
          </Link>
        )}
      </div>
    </div>
