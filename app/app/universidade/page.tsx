    ...
    <div className="space-y-8 animate-fade-in">
      ...
      >
        ...
        <div className="relative z-10">
          <p className="text-accent text-xs font-semibold tracking-[0.15em] uppercase mb-2">Universidade</p>
      ...
      {novosConteudos.length > 0 && (
        <section>
          <h2 className="font-serif text-xl font-semibold text-ink mb-4 flex items-center gap-2">
            <Sparkles size={18} strokeWidth={1.75} className="text-accent" /> Novos conteúdos
          </h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <section>
        <h2 className="font-serif text-xl font-semibold text-ink mb-4 flex items-center gap-2">
          <Video size={18} strokeWidth={1.75} className="text-primary" /> Aulas rápidas
        </h2>
      ...
      <section>
        <h2 className="font-serif text-xl font-semibold text-ink mb-1 flex items-center gap-2">
          <Video size={18} strokeWidth={1.75} className="text-primary" /> Cases de venda
        </h2>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {depoimentosLojistas.map((d) => (
            <div key={d.id} className="card-fassi p-5 flex items-start gap-4">
              <div className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "linear-gradient(135deg, #1F1C18 0%, #3D3833 100%)" }}>
                <Play size={20} strokeWidth={1.75} className="text-white/80 ml-1" fill="currentColor" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted mt-1 flex items-center gap-1">
                  <Clock size={11} strokeWidth={1.75} /> {d.duracao}
                </div>
              </div>
            </div>
          ))}
      ...
      <section>
        <h2 className="font-serif text-xl font-semibold text-ink mb-4 flex items-center gap-2">
          <Download size={18} strokeWidth={1.75} className="text-primary" /> Biblioteca de materiais
        </h2>
          ...
          {bibliotecaMateriais.map((m) => {
            ...
            return (
              <div key={m.id} className="card-fassi p-4 group cursor-pointer">
                ...
                <button className="text-xs font-semibold text-primary mt-2 inline-flex items-center gap-1">
      ...
      <section>
        <h2 className="font-serif text-xl font-semibold text-ink mb-4 flex items-center gap-2">
          <Calendar size={18} strokeWidth={1.75} className="text-primary" /> Agenda da Universidade
        </h2>
              ...
              <div key={a.id} className="card-fassi p-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[0.65rem] px-2 py-0.5 rounded-full font-medium bg-primary/10 text-primary">
                  ...
                  <div className="text-xs text-muted flex items-center gap-3">
                    <span className="flex items-center gap-1"><Calendar size={11} strokeWidth={1.75} /> {a.data}</span>
                    <span className="flex items-center gap-1"><Clock size={11} strokeWidth={1.75} /> {a.horario}</span>
                  </div>
                </div>
              </div>
            );
      ...
      {role === "lojista" && (
        <section>
          ...
          >
            <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
              <Users size={20} strokeWidth={1.75} className="text-primary" />
            </div>
            ...
            <ArrowRight size={18} strokeWidth={1.75} className="text-muted group-hover:text-primary transition-colors" />
          </Link>
        </section>
      )}
    </div>
      ...
      <div className="card-fassi overflow-hidden group cursor-pointer h-full flex flex-col">
        ...
        >
          ...
          <div className="relative z-10 flex items-center gap-2">
            {curso.novo && (
              <span className="text-[0.6rem] px-2 py-0.5 rounded-full bg-accent text-[#1F1C18] font-bold">
                NOVO
              </span>
            )}
            {curso.essencial && (
            ...
            {concluido && (
              <span className="text-[0.6rem] px-2 py-0.5 rounded-full bg-white/20 text-white font-medium flex items-center gap-1">
                <Check size={9} strokeWidth={1.75} /> CONCLUÍDO
              </span>
            )}
          </div>
        </div>
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-3 text-[0.7rem] text-muted mb-3">
            <span className="flex items-center gap-1"><BookOpen size={11} strokeWidth={1.75} /> {totalModulos} mód.</span>
            <span className="flex items-center gap-1"><Play size={11} strokeWidth={1.75} /> {totalAulas} aulas</span>
            <span className="flex items-center gap-1"><Clock size={11} strokeWidth={1.75} /> {curso.duracaoTotal}</span>
          </div>
          ...
          <div className="flex items-center justify-between text-[0.7rem] mb-3">
            ...
            {curso.certificado && (
              <span className="flex items-center gap-1 text-accent-dark font-medium">
                <Award size={11} strokeWidth={1.75} /> Certificado
              </span>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-line">
            <span className={cn(
              ...
              concluido ? "text-muted" : "text-primary"
            )}>
